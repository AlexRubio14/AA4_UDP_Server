...
void EventManager::UDPEmit(const PacketType type, CustomUDPPacket customPacket, sf::IpAddress senderIpAdress, int senderPort)
{
    ...
    if (it != UDPsubscribers.end()) {
        for (UDPCallback& callback : it->second) {
            callback(customPacket, senderIpAdress, senderPort);
        }
    }
    else
